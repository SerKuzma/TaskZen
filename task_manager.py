...
def read_user_data():
    # Checks if "user.txt" file exists, if not, creates it with default admin user
...
def register_user(username_password):
    ...
    if new_username in username_password:
        print(Colors.RED + "User already exists" + Colors.RESET)
        return
    
    # Asks for the new password and confirms it
    new_password = input(Colors.YELLOW + "New Password: " + Colors.RESET)
    confirm_password = input(Colors.YELLOW + "Confirm Password: " + Colors.RESET)
    ...
    if new_password == confirm_password:
        ...
        with open("user.txt", "w") as out_file:
            ...
            for username in username_password:
                user_data.append(f"{username};{username_password[username]}")
            out_file.write("\n".join(user_data))
...
def delete_user(username_password, task_list):
    # Input prompt for the username to be deleted
    username_to_delete = input(Colors.YELLOW + "Enter the username you want to delete: "
    + Colors.RESET)

    # Checks if the current user is admin & enhance admin's rights
    if curr_user != 'admin':
        print(Colors.RED + "Users can be deleted by the admin only." + Colors.RESET)
        return
    ...
    if username_to_delete in username_password:
        ...
        # Updates the user.txt file with the modified username_password dictionary
        with open("user.txt", "w") as out_file:
            user_data = [
            f"{username};{password}" for username, password in username_password.items()]
            out_file.write("\n".join(user_data))
        ...
        # Handle the case where tasks.txt does not exist
        try:
            task_list = read_tasks_from_file("tasks.txt")

        except FileNotFoundError:
            # If tasks.txt does not exist, print a message and return
            print(Colors.RED + "Tasks file not found. No tasks to delete." + Colors.RESET)
            return
...
def add_task(task_list, username_password):
    ...
    task_description = input(Colors.YELLOW + "Description of Task: " + Colors.RESET)

    # Prompts the user for the due date until a valid date is entered
    while True:
        try:
            task_due_date = input(Colors.YELLOW + "Due date of task (YYYY-MM-DD): " 
            + Colors.RESET)
            due_date_time = datetime.strptime(task_due_date, DATETIME_STRING_FORMAT)
            # Checks if the due date is in the past
            if due_date_time.date() < date.today():
                print(Colors.RED + "Sorry, you can't add overdue task." + Colors.RESET)
                continue
            break

        except ValueError:
            print(Colors.RED + "Invalid datetime format. Please use the format specified" 
            + Colors.RESET)
...
def read_tasks_from_file(file_path):
    ...
    try:
        with open(file_path, 'r') as task_file: 
            # Read the content of the file and split it by newline character
            task_data = task_file.read().split("\n")
            # Remove empty strings from the list
            task_data = [t for t in task_data if t != ""]

        # Iterate over each task string in the task_data list
        for t_str in task_data:
            curr_t = {}  # Initialize an empty dictionary for the current task
            task_components = t_str.split(";")

            # Assign values to keys in the curr_t dictionary based on task components
            curr_t['username'] = task_components[0]
            curr_t['title'] = task_components[1]
            curr_t['description'] = task_components[2]
            # Convert the due date string to datetime object using specified format
            curr_t['due_date'] = datetime.strptime(task_components[3],
            DATETIME_STRING_FORMAT)
            # Convert the assigned date string to datetime object using specified format
            curr_t['assigned_date'] = datetime.strptime(task_components[4], 
            DATETIME_STRING_FORMAT)
            # Set the 'completed' key based on the string value ('Yes' or 'No')
            curr_t['completed'] = True if task_components[5] == "Yes" else False
            # Append the current task dictionary to task_list
...
def color_due_date(date, today):
    ...
    """
    if date < today:
        return f"{Fore.RED}{date}{Style.RESET_ALL}"
    elif date == today:
...
def view_all(task_list):
    ...
    for i, t in enumerate(task_list, start=1):
        ...
        # Initialize a string to display task information
        disp_str = Fore.GREEN + f"Task # {i}:\n" + Style.RESET_ALL
        disp_str += f"Task: \t\t {t['title']}\n"
        disp_str += f"Assigned to: \t {Fore.MAGENTA}{t['username']}{Style.RESET_ALL}\n"
    ...
    while True:
        ...
        for i, t in enumerate(task_list, start=1):
            ...
            if t['username'] == curr_user:
                today = datetime.today().date()
                # Construct the display string for the task
        ...
        # Prompt the user to select a task for editing or return to the main menu
        try:
            task_number = int(input(Colors.YELLOW + 
            "Enter the number of the task you want to edit, or" 
            + Colors.BLUE + " 0 " + Colors.RESET + Colors.YELLOW + "for main menu: " 
            + Colors.RESET))
        ...
        if selected_task['completed']:
            print(Colors.RED + "Sorry, you can't edit completed tasks" + Colors.RESET)
            print()
            continue
        
        # Check if the selected task is overdue
        if selected_task['due_date'].date() < datetime.today().date():
            print(Colors.RED + "Sorry, you can't edit overdue tasks." + Colors.RESET)
            print() 
            continue

        edit_option = input(Colors.YELLOW + "Do you want to edit this task? (y/n): " 
        + Colors.RESET).lower()

        # Execute program to edit task 
        if edit_option == 'y':
...
def edit_task(task, task_list):
    ...
    if task['completed']:
        print(Colors.RED + "This task is already completed. You cannot modify it." 
        + Colors.RESET)
        print()
        return
    ...
    while True:
        ...
        action_choice = input(Colors.YELLOW + "Enter the number corresponding to" \
        ...
        # Modify the username of the task
            ...
            new_username = input(Colors.YELLOW + "Enter the new username: " + Colors.RESET)
         ...
         # Modify the due date of the task 
            ...
            while True:
                try:
                    ...
                    # Parse the user input as a datetime object using the specified format
                    due_date_time = datetime.strptime(task_due_date, DATETIME_STRING_FORMAT)

                    # Check if the new due date is in the past
                    if due_date_time.date() < date.today():
                        print(Colors.RED + "Sorry, you can't edit task to overdue"
                        + Colors.RESET)
        ...
        # Return to the main menu
            ...
            print(Colors.RED + "Invalid choice. No changes made." + Colors.RESET)
            print()
            continue
...
def update_task_in_file(task_list):
    with open("tasks.txt", "w") as task_file:
        ...
        for t in task_list:
            ...
            # Joins the task attributes with ';' and appends to the task_list_to_write
...
def generate_task_overview(task_list):
    ...
    with open("task_overview.txt", "w") as task_overview_file:
        ...
        task_overview_file.write(f"Total number of tasks not completed and due to overdue: "
        f"{overdue_tasks}\n")
...
def display_statistics(curr_user, username_password):
    ...
    if not os.path.exists("tasks.txt") or not os.path.exists("user.txt"):
        ...
        # Create a user.txt file with admin credentials
...
def generate_user_overview(username_password, task_list):
    ...
    with open("user_overview.txt", "w") as user_overview_file:
        ...
        for username, password in username_password.items():
            ...
            percentage_completed_user_tasks = (completed_user_tasks / total_user_tasks
            ) * 100 if total_user_tasks > 0 else 0
            percentage_must_completed = (incomplete_user_tasks / total_user_tasks
            ) * 100 if total_user_tasks > 0 else 0
            percentage_overdue_user_tasks = (overdue_user_tasks / total_user_tasks
            ) * 100 if total_user_tasks > 0 else 0
            ...
            user_overview_file.write(
            f"% of tasks assigned and completed: {percentage_completed_user_tasks:.2f}%\n")
            user_overview_file.write(
...
while True:
    ...
    # In the block below calls functions depends on user's choice; errors handling
    if menu == 'r':
